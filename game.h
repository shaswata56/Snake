	...
	while (window.isOpen())
	{
		...
		if (timer>delay)                                                       // 0.1 second por por saaper position agano
		{
            ...
            if ((s[0].x == f.x) && (s[0].y == f.y))         //saap khabar khaise naki oita check korar jonno
            {
                ...
                for(int i=0; i<108; i++)
                {
                    if(wl[i].x == f.x && wl[i].y == f.y) goto a;

                    if(i>=7 && i<=41 && i == f.x && 25 == f.y ) goto a;
                }
            }
            ...
            if(num%7==0)
            {
                if(tle==0)
                {
                    ...
                    for(int i=0; i<108; i++)
                    {
                        if(wl[i].x == bf.x && wl[i].y == bf.y) goto b;

                        if(i>=7 && i<=41 && i == bf.x && 25 == bf.y ) goto b;
                    }
                }
                tle++;
            }
		...
		for(int i=0;i<108;i++)
        {
            ...
            window.draw(sWall);
            if(i>=7 && i<=41) sWall.setPosition( i*size, 25*size );
            window.draw(sWall);
        }
		...
		if((tle<50)&&(num%7==0)) window.draw(sBon);
